...
func NewServer(assets string, facebox *facebox.Client) *Server {
	...
	srv.router.HandleFunc(http.MethodPost, "/facebox/teach", srv.handleFaceboxTeach)
...
func (s *Server) handleFaceboxTeach(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		http.Error(w, "something went wrong teaching", http.StatusInternalServerError)
		return
	}
	w.Write([]byte("OK"))
}
